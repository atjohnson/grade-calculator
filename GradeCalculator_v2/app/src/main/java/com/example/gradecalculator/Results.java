...
public class Results extends AppCompatActivity implements View.OnClickListener {
    ...
    private void DisplayGrades()
    {
        ...
        double homeworkGrade = getIntent().getExtras().getDouble(Projects.TAG_HW_GRADE);
        double homeworkTotal = getIntent().getExtras().getDouble(Projects.TAG_HW_TOTAL);
        //Gets values pushed from quiz section
        double quizGrade = getIntent().getExtras().getDouble(Projects.TAG_QUIZ_GRADE);
        double quizTotal = getIntent().getExtras().getDouble(Projects.TAG_QUIZ_TOTAL);
        //Gets values pushed from labs section
        double labGrade = getIntent().getExtras().getDouble(Projects.TAG_LAB_GRADE);
        double labTotal = getIntent().getExtras().getDouble(Projects.TAG_LAB_TOTAL);
        //Gets values pushed from exam section
        double examGrade = getIntent().getExtras().getDouble(Projects.TAG_EXAM_GRADE);
        double examTotal = getIntent().getExtras().getDouble(Projects.TAG_EXAM_TOTAL);
        ...
        String totGrade = String.format("%.2f", totalGrade);
        gradeText = gradeText + totGrade +"%";
        ...
        String hwGrade = String.format("%.2f", homeworkGrade);
        hwText = hwText + hwGrade +"%";
        homeworkText.setText(hwText);

        //Displays quiz grade
        String qzText = quizzesText.getText().toString();
        String qzGrade = String.format("%.2f", quizGrade);
        qzText = qzText + qzGrade +"%";
        quizzesText.setText(qzText);

        //Displays lab grade
        String labText = labsText.getText().toString();
        String lbGrade = String.format("%.2f", labGrade);
        labText = labText + lbGrade +"%";
        labsText.setText(labText);

        //Displays exam grade
        String exText = examsText.getText().toString();
        String exmGrade = String.format("%.2f", examGrade);
        exText = exText + exmGrade +"%";
        examsText.setText(exText);

        //Displays project grade
        String proText = projectsText.getText().toString();
        String prjGrade = String.format("%.2f", projectGrade);
        proText = proText + prjGrade +"%";
